...
const promotionGames = computed(() => {
  return games.value.filter(g => g.tipoPromocion === 'promocion')
})
    ...
    <main class="py-12">
      <div class="container mx-auto px-4 sm:px-6">
        ...
        <div class="flex items-center gap-3 mb-8">
          <div class="p-2 bg-pink-600/10 rounded-lg">
            <Tag class="text-pink-500" :size="24" />
          </div>
          <div>
            <h1 class="text-3xl font-bold text-white">Promociones</h1>
            <p class="text-gray-400">Juegos con promociones especiales</p>
          </div>
        </div>
        ...
        <div v-else-if="promotionGames.length > 0" class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-4 sm:gap-6">
          <GameCard
            v-for="game in promotionGames"
            :key="game.id"
            :game="game"
        ...
        <div v-else class="text-center py-20">
          <p class="text-xl text-gray-500">No hay promociones activas en este momento.</p>
        </div>
      </div>
    </main>
